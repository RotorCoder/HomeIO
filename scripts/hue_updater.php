...
try {
    $log->logInfoMsg("Starting Hue device updater");
    ...
    while (true) {
        try {
            ...
            curl_setopt_array($curl, [
                CURLOPT_URL => "https://mittencoder.com/homeio/api/update-hue-devices",
            ...
            if ($httpCode === 200) {
                ...
                if ($result['success']) {
                    ...
                    $log->logInfoMsg("Updated $deviceCount Hue devices. API timing: {$result['timing']['devices']['duration']}ms, DB timing: {$result['timing']['states']['duration']}ms");
                }
            } else {
            ...
            sleep(5); // Still sleep on error to prevent rapid error loops
        }
    }
